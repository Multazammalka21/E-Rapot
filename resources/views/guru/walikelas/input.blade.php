    ...
    <div class="panel-body">
        <form action="{{ route('guru.walikelas.store', $kelas->id) }}" method="POST">
            ...
            <div class="table-responsive" style="overflow-x:auto;-webkit-overflow-scrolling:touch">
                <table class="table" style="width:100%;table-layout:fixed;min-width:580px;border-collapse:collapse">
                    <colgroup>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center" title="Tanpa Keterangan (Alpha)">A</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $sk->nomor_urut }}</td>
                            <td>
                                <div style="font-weight:600;font-size:0.875rem">{{ $sk->siswa?->nama_lengkap }}</div>
                                <div style="font-size:0.75rem;color:var(--text-muted)">NIS: {{ $sk->siswa?->nis }}</div>
                            </td>
                            <td style="text-align:center;padding:0.4rem 0.25rem">
                                <input type="number"
                                    name="data[{{ $sk->siswa_id }}][hadir]"
                                    class="form-control"
                                    value="{{ old('data.'.$sk->siswa_id.'.hadir', $ab->hadir ?? 0) }}"
                                    min="0" required
                                    style="width:48px;padding:0.35rem 0.2rem;text-align:center;font-size:0.85rem;font-family:inherit;border-radius:6px;margin:0 auto">
                            </td>
                            <td style="text-align:center;padding:0.4rem 0.25rem">
                                <input type="number"
                                    name="data[{{ $sk->siswa_id }}][sakit]"
                                    class="form-control"
                                    value="{{ old('data.'.$sk->siswa_id.'.sakit', $ab->sakit ?? 0) }}"
                                    min="0" required
                                    style="width:48px;padding:0.35rem 0.2rem;text-align:center;font-size:0.85rem;font-family:inherit;border-radius:6px;margin:0 auto">
                            </td>
                            <td style="text-align:center;padding:0.4rem 0.25rem">
                                <input type="number"
                                    name="data[{{ $sk->siswa_id }}][izin]"
                                    class="form-control"
                                    value="{{ old('data.'.$sk->siswa_id.'.izin', $ab->izin ?? 0) }}"
                                    min="0" required
                                    style="width:48px;padding:0.35rem 0.2rem;text-align:center;font-size:0.85rem;font-family:inherit;border-radius:6px;margin:0 auto">
                            </td>
                            <td style="text-align:center;padding:0.4rem 0.25rem">
                                <input type="number"
                                    name="data[{{ $sk->siswa_id }}][alpha]"
                                    class="form-control"
                                    value="{{ old('data.'.$sk->siswa_id.'.alpha', $ab->alpha ?? 0) }}"
                                    min="0" required
                                    style="width:48px;padding:0.35rem 0.2rem;text-align:center;font-size:0.85rem;font-family:inherit;border-radius:6px;margin:0 auto">
                            </td>
                            <td style="padding:0.4rem 0.5rem">
                                <textarea
                                    name="data[{{ $sk->siswa_id }}][catatan]"
                                    class="form-control"
                                    rows="2"
                                    placeholder="Catatan untuk siswa..."
                                    style="width:100%;box-sizing:border-box;padding:0.4rem 0.6rem;font-size:0.82rem;font-family:inherit;line-height:1.5;resize:vertical;border-radius:6px">{{ old('data.'.$sk->siswa_id.'.catatan', $cat->catatan ?? '') }}</textarea>
                            </td>
                        </tr>
                        @endforeach
